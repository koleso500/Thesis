...
plt.savefig("plots/SEM_ROC_Curve_ny_original.png", dpi=300)
plt.close()
...
plt.title("Stacked Ensemble Model RGE Curve (New York Original)")
plt.legend()
plt.grid(True)
plt.savefig("plots/SEM_RGE_ny_original.png", dpi=300)
plt.close()
...
plt.title('Stacked Ensemble Model RGR Curve (New York Original)')
...
plt.xlim([0, 1])
plt.grid(True)
plt.savefig("plots/SEM_RGR_ny_original.png", dpi=300)
plt.close()

# Fairness
gender = compute_rga_parity(x_train, x_test, y_test, y_prob, stacking_clf, "applicant_sex")
print("Gender:\n", gender)
