...
x_train, x_test, y_train, y_test = train_test_split(predictors, y, test_size=0.2, random_state=20)
...
plt.savefig("plots/LR_ROC_Curve_ny_original.png", dpi=300)
plt.close()
...
plt.legend()
plt.grid(True)
plt.savefig("plots/LR_RGE_ny_original.png", dpi=300)
plt.close()
...
plt.xlim([0, 1])
plt.grid(True)
plt.savefig("plots/LR_RGR_ny_original.png", dpi=300)
plt.close()
...
file_path = os.path.join("../saved_data", "final_results_lr_ny_original.json")
