...
from safeai_files.core import rga, partial_rga_with_curves
...
plt.savefig("plots/VEM_ROC_Curve_ny_article.png", dpi=300)
plt.close()
...
for k in range(0, len(explain) + 1):
    if k == 0:
        rge_k = compute_rge_values(x_train, x_test, y_prob, voting_clf, remaining_vars, group=True)
...
plt.ylabel("RGE")
plt.title("Voting Ensemble Model RGE Curve (New York Article)")
plt.legend()
plt.grid(True)
plt.savefig("plots/VEM_RGE_ny_article.png", dpi=300)
plt.close()
...
plt.title('Voting Ensemble Model RGR Curve (New York Article)')
...
plt.xlim([0, 1])
plt.grid(True)
plt.savefig("plots/VEM_RGR_ny_article.png", dpi=300)
plt.close()
...
file_path = os.path.join("../saved_data", "final_results_voting_ny_article.json")
