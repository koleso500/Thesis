...
def evaluate_model_ny_article():
    ...
    for k in range(0, len(explain) + 1):
        if k == 0:
            rge_k = compute_rge_values(x_train_scaled_names, x_test_scaled_names, y_pred_prob, best_model, remaining_vars, group=True)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("plots/NN_RGE_ny_article.png", dpi=300)
    plt.close()
    ...
    plt.xlim([0, 1])
    plt.grid(True)
    plt.savefig("plots/NN_RGR_ny_article.png", dpi=300)
    plt.close()
