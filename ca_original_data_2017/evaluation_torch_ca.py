...
# Load loss values
loss_values = np.load(os.path.join(save_dir, "loss_values_ca.npy"))
...
def evaluate_model_ca():
    ...
    single_rge = compute_rge_values(x_train_scaled_names, x_test_scaled_names, y_pred_prob, best_model, explain)
    print(single_rge)
    ...
    for k in range(0, len(single_rge) + 1):
        ...
        rge_k = compute_rge_values(x_train_scaled_names, x_test_scaled_names, y_pred_prob, best_model, top_k_vars,
                                   group=True)
    ...
    plt.ylabel("RGE")
    plt.title("Neural Network RGE Curve (California)")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    ...
    plt.title('Neural Network RGR Curve (California)')
